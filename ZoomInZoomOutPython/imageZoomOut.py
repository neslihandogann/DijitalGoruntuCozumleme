...
#degeri küçüldükçe resimi uzaklaştırmayı sağlıyoruz.
def zoom_function(img, zoom_value=0.3):
...
#resimlerdeki satır ve sutün  dizilimleri ve matrix mantığı ile bir dizi linner işlemler doğrultusunda zoom değeri ile oynayarak resimi uzaklaştırmayı sağladık yani zoom out işlemi gerçekleşmiş oldu.
    x1 = int(0.7*n*(1-1/zoom_value))
    x2 = int(n-7.2*n*(1-1/zoom_value))
    y1 = int(0.7*m*(1-1/zoom_value))
    y2 = int(m-0.7*m*(1-1/zoom_value))
...
cv2.imshow("zoom out",img_zoomed) #resim fonksiyon uygulanarak ekranda yakınlaşmış şekilde görünecektir
